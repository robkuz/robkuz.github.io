...
and you might have thought to yourself I don't need no stickin' HKT 
...
You start with your PM the first design/coding session - It's about line items. And very soon you 2 come up with the following design.
...
That was a productive day and somehow the PM was nice and understanding. Maybe he isn't that evil guy you always pictured him?
...
You wonder why he read the anaysis docs only now. Effectively he explains to you that the `LineItem` needs to be processed in stages thru out the program.  
There are 2 stages: `InProgress` and `Finished`. While `InProgress` any property can be changed by the system.  
For example we might reduce the units because there aren't enough on stock or we reduce the amount because our client is getting some rebate etc.
So you start creating 2 types

``` fsharp
type InProgress<'a> = {value: 'a; decisions: List<Decision>}
...
So you open your editor to reuse your already existing code - only there is no reuse (except you have put "copy & paste" into your mental category of "reuse"). 
...
The PM cuts the meeting short as he sees the murderous look building in your eyes without telling you about those `Archived` and `Prepared` states.
...
Before I explain how to emulate HKT in F# in Part III of this series lets look briefly into Haskell and how they solve this in Part II
